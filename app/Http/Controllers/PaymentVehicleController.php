...
class PaymentVehicleController extends Controller
{
    public function successVehiclePayment(Request $request) {
        session_start(); 
        ...
        // echo '<pre>';
        // print_r(Session::get('productVehicleData'));
        // echo "successFeaturePayment";
        if( null == (Session::get('productVehicleData')) ) {
        ...
        $productData = Session::get('productVehicleData');
        if(isset($productData['client_reference_id'])) {
            ...
            ->pluck('count');
                ...
                $vehicle_count = $order_vehicle_count->order_vehicle_count + $fcount;
                /**
    ...
    public function cancelVehiclePayment(Request $request) {
        session_start(); 
        if(empty($_SESSION['login']))
        {
            header("location: https://fna2.phpapplord.co.za/public/");
    ...
    public function notifyVehiclePayment(Request $request) {
        session_start(); 
        if(empty($_SESSION['login']))
        {
            header("location: https://fna2.phpapplord.co.za/public/");
