...
class ClientLiabilitiesController extends Controller
{
    ...
    //  dd();
        ...
        $my_assets = DB::table('client_assets')
                        // ->select('asset_sub_type', 'id')
                        ->where('client_reference_id', '=', $client_reference_id)
                        ->orderBy('id','DESC')
                        ->get(['asset_sub_type','id', 'client_type']);
    ...
    public function storeClientLiablilities(Request $request)
    {
        ...
        DB::table('audit')->insert([
            ...
            'action' => "Landed on Store liabilities page",
		...
		[
			...
			'loan_term_value' => intval(preg_replace('/[^\d.]/', '', $_POST['loan_term_value'])),
        ...
        //                 WHERE
        //                     client_reference_id = '". $client_reference_id ."' ORDER BY client_assets.id DESC");
        // $result2 = DB::select("SELECT
        //                             client_liabilities_new.id,
        ...
        //                         WHERE
        //                             client_reference_id = '". $client_reference_id ."' ORDER BY client_liabilities_new.id DESC"); 
...
public function editLiabilityNew($clientReff, $id){
    ...
    DB::table('audit')->insert([
        ...
        'module' => $personalInfoModule,
        'role' => $userRole[0]->name,
        'action' => "Landed on Edit liabilities page",
        'date' => DB::raw('now()')
    ]); 
    ...
    $my_assets = DB::table('client_assets')
                ...
                ->get(['asset_sub_type','id', 'client_type']);
   //dd($client_owners);
    return view('fna.editLiablilityNew',['client_assets' => $my_assets, 'liability_types' => $liability_types, 'liability'=>$liability[0],'asset_owners' => $liability_owners, 'client_reference_id'=>$clientReff, 'client_owners'=>$client_owners, 'liability_owners' => $arrOwners]);
}
...
public function update_Liability(Request $request){
     $request->validate([
        ...
        DB::table('audit')->insert([
            ...
            'module' => $personalInfoModule,
            'role' => $userRole[0]->name,
            'action' => "Landed on Update liabilities page",
            'action_details' => json_encode($request->all()),
            'date' => DB::raw('now()')
        ]); 
        $user_id = 1;
