...
class PaymentCpbAssetController extends Controller
{
    public function successCpbAssetPayment(Request $request) {
        session_start(); 
        ...
        if(isset($productData['client_reference_id'])) {
            ...
            $features_count = DB::table('extra_purchase')     
                ...
                ->pluck('count');
            $asset_count = 0;
            $order_asset_count = DB::table('orders')
                                        ->where('user_id', $user_id)
                                        ->orderBy('id', 'DESC')
                                        ->first(['order_cpb_asset_count','id']);
            $asset_count = $order_asset_count->order_cpb_asset_count + $features_count[0];
            ...
            DB::table('orders')
                    ->where('user_id', $user_id)
                    ->where('id',$order_asset_count->id)
                    ->update([
                    'order_cpb_asset_count' => $asset_count
                ]);
    ...
    public function cancelCpbAssetPayment(Request $request) {
        session_start(); 
        if(empty($_SESSION['login']))
        {
            header("location: https://fna2.phpapplord.co.za/public/");
        ...
        if( null == (Session::get('productPropertyData')) ) {
            return redirect()->route('whatamiworth', ['client_reference_id' => $_SESSION['client_reference_id'], 'client_type' => 'Main Client']);
        }        
        $productData = Session::get('productCpbAssetData');
        if(isset($productData['client_reference_id'])) {
            $user_id = $productData['client_reference_id'];
    ...
    public function notifyCpbAssetPayment(Request $request) {
